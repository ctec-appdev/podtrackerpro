...
const STRIPE_PRODUCTS = {
  starter: "prod_U6hv1KmiUmALuX",
  business: "prod_U6hwHlnEuJdmPm",
};
...
async function askClaudeJSON(prompt, sys) {
  try {
    const r = await fetch("/api/claude", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
function SEOView({ data, addItem, deleteItem, loading, setLoading, plan, usage, setUsage }) {
  ...
  const generate = async () => {
    ...
    const res = await askClaudeJSON(prompt, "You are a POD SEO copywriting expert. Write compelling, keyword-rich copy.");
    setGenerated(res);
    setLoading(false);
  };
